    ...
    public class EditorCoroutine
    {
        private struct YieldProcessor
        {
            ...
            public void Set(object yield)
            {
                ...
                if (type == typeof(EditorWaitForSeconds))
                {
                    targetTime =
                        ...
                        + (yield as EditorWaitForSeconds).WaitTime;
                ...
                else if (type == typeof(EditorCoroutine))
                {
                    ...
                    type == typeof(AsyncOperation)
                    || type.IsSubclassOf(typeof(AsyncOperation))
                )
                {
            ...
            public bool MoveNext(IEnumerator enumerator)
            {
                ...
                {
                    DataType.WaitForSeconds
                        => data.targetTime <= EditorApplication.timeSinceStartup,
        ...
        private bool ProcessIEnumeratorRecursive(IEnumerator enumerator)
        {
            ...
            if (
                kIEnumeratorProcessingStack.Count > 0
                && !result
                && root == kIEnumeratorProcessingStack.Pop()
            )
            {
